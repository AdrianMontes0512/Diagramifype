...
// Actualizar producto usando PUT con SKU en la URL
export const ActualizarProducto = async (producto: Product): Promise<Product> => {
  try {
    const token = localStorage.getItem('token');
    
    // Encode el SKU para manejar caracteres especiales
    const encodedSku = encodeURIComponent(producto.sku || '');
    ...
    const response = await API_PRODUCTS.put(`/productos/inkafarma/${encodedSku}`, productData);
